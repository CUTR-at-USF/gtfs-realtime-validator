 ...
 * `-plaintext` *(Optional)* - If this argument is supplied, the validator will output a plain text version of each of the protocol buffer files with the provided file extension.  For example, if the protocol buffer file has the name "file.pb", and the text "txt" is provided as the argument, then the plain text version of this file will be "file.pb.txt".
